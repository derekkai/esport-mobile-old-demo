...
import { PUSH_TEST_DATA, openModal } from 'reducers/global';
import { unsubscribe } from 'helpers/commandSender';
import settings from 'settings';
import testData from './market.json';
...
function* initializeWebSocketsChannel() {
  try {
    ...
    console.log(e);
    // colorLog('Reconnect after 10 seconds!', 'error');
    // yield delay(settings.serverReconnectingIntervalTime);
    // yield call(initializeWebSocketsChannel);
  }
}

function* pushData() {
  const temp = Object.entries(testData.data);
  const subid = temp[0][0];
  const others = temp[0][1];
  console.log(subid);
  console.log(others);
...
export default function* watchers() {
  ...
  // );
  yield takeEvery(PUSH_TEST_DATA, pushData);
}
